 ...
 * Reference UI: legacy MIS Date-tab form
...
function esc(mysqli $db, $val): string { return $db->real_escape_string(trim((string)$val)); }
function h(string $val): string { return htmlspecialchars($val, ENT_QUOTES, 'UTF-8'); }
function safeQuery(mysqli $db, string $sql): array {
    $r = $db->query($sql);
    if (!$r) return [];
    $rows = [];
...
function multiSelected(array $selected, string $value): string {
    return in_array($value, $selected, true) ? 'checked' : '';
}
function fmtNum($v, int $dec = 2): string { return number_format((float)$v, $dec); }
function asList(mysqli $db, $raw): array {
    if (is_array($raw)) {
...
function inListSql(mysqli $db, array $vals): string {
    return implode(',', array_map(fn($v) => "'" . esc($db, $v) . "'", $vals));
}
...
<?php
// Helper to render a multiselect filter block used by other tabs
function renderMs(
    string $id, string $label, string $name, array $dd, string $valKey, string $labelKey,
    array $selected, string $allText
): void {
?>
  <div class="fg-col-ms">
    <span class="fl"><?= h($label) ?></span>
    <div class="ms-dropdown" id="<?= h($id) ?>">
      <div class="ms-trigger" onclick="toggleMs('<?= h($id) ?>')">
        ...
        ?></span>
        <?php if (!empty($selected)): ?><span class="ms-badge"><?= count($selected) ?></span><?php endif; ?>
      </div>
      <div class="ms-menu" id="<?= h($id) ?>-menu">
        <div class="ms-search"><input type="text" placeholder="Search…" oninput="msFilter('<?= h($id) ?>',this.value)"></div>
        <?php foreach ($dd as $r):
            $val = (string)($r[$valKey] ?? '');
        ...
        ?>
        <div class="ms-item">
          <input type="checkbox" name="<?= h($name) ?>[]" value="<?= h($val) ?>" id="<?= h($cid) ?>" <?= multiSelected($selected, $val) ?>>
          <label for="<?= h($cid) ?>"><?= h($lab) ?></label>
        </div>
        <?php endforeach; ?>
        <div class="ms-actions">
          <button type="button" class="btn btn-outline-secondary btn-xs" onclick="msSelectAll('<?= h($id) ?>',true)">All</button>
          <button type="button" class="btn btn-outline-secondary btn-xs" onclick="msSelectAll('<?= h($id) ?>',false)">None</button>
          <button type="button" class="btn btn-primary btn-xs" onclick="toggleMs('<?= h($id) ?>')">Done</button>
        </div>
      </div>
    </div>
  </div>
