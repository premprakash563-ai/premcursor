...
set -euo pipefail
cd ~/public_html
mkdir -p custom/include/BS cache custom/application/Ext/EntryPointRegistry custom/Extension/application/Ext/EntryPointRegistry
cat > bs_fix.php << 'ENDOFFILE'
<?php
/**
...
ENDOFFILE
cat > custom/include/BS/retrieve_dash_page_safe.php << 'ENDOFFILE'
<?php
/**
...
if [ -f custom/include/MVC/Controller/entry_point_registry.php ]; then
  mv -f custom/include/MVC/Controller/entry_point_registry.php custom/include/MVC/Controller/entry_point_registry.php.bak.$(date +%s)
fi
for f in modules/Home/Dashlets/BS_AdminDashboardDashlet/BS_AdminDashboardDashlet.php modules/BS_Orders/Dashlets/BS_EmployeeWorkloadDashlet/BS_EmployeeWorkloadDashlet.php; do
  [ -f "$f" ] && mv -f "$f" "${f}.off"
done
rm -f cache/dashlets/dashlets.php
grep -n "BS FIX start v2" bs_fix.php
php -r 'if (function_exists("opcache_reset")) opcache_reset();' 2>/dev/null || true
echo "OK written. Open: https://yoogleconsultancy.in/bs_fix.php?v=2"
