<?php
/**
 * Shared Operations Board HTML renderer (body only).
 */
...
class BS_BoardRenderer
{
    public function renderBody()
    {
        ...
        echo '<header class="bs-board__hero"><div>';
        echo '<p class="bs-board__eyebrow">Yoogle Consultancy · Business Service CRM</p>';
        echo '<h1>Operations Board</h1>';
        echo '<p class="bs-board__lead">Clean live view of today\'s orders, pipeline health, and team load — built for daily ops.</p>';
        echo '</div><div class="bs-board__hero-actions">';
        ...
        echo '<section class="bs-board__grid"><div class="bs-panel">';
        echo '<div class="bs-panel__head"><h2>Pipeline</h2><span>Open work by stage</span></div><ul class="bs-pipe">';
        foreach ($stats['pipeline'] as $label => $count) {
            $base = max((int) $stats['pending'], 1);
            $pct = ($label === 'Ready / Delivered') ? 0 : (int) round(($count / $base) * 100);
            echo '<li><div class="bs-pipe__row"><span>' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</span><strong>' . (int) $count . '</strong></div>';
            echo '<div class="bs-pipe__bar"><i style="width:' . min(100, $pct) . '%"></i></div></li>';
        }
        echo '</ul></div><div class="bs-panel">';
        ...
        echo '<footer class="bs-board__foot">Signed in as ' . htmlspecialchars($who, ENT_QUOTES, 'UTF-8') . ' · Custom Operations Board</footer>';
        echo '</div>';
    }

    protected function kpi($label, $value, $hint, $href)
    {
        return '<a class="bs-kpi" href="' . htmlspecialchars($href, ENT_QUOTES, 'UTF-8') . '"><span>'
            . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '</span><strong>'
            . htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . '</strong><em>'
            . htmlspecialchars($hint, ENT_QUOTES, 'UTF-8') . '</em></a>';
    }
...
.bs-board{--ink:#122033;--muted:#5b6b7c;--line:#d7e0ea;--accent:#0f766e;--soft:#e7f5f3;font-family:"IBM Plex Sans",system-ui,sans-serif;color:var(--ink);background:linear-gradient(165deg,#e8eef4 0%,#f7f9fb 45%,#fff 100%);border:1px solid var(--line);border-radius:14px;padding:28px;margin:8px 0 24px;box-shadow:0 10px 30px rgba(18,32,51,.06)}
...
.bs-kpi{background:#fff;border:1px solid var(--line);border-radius:12px;padding:16px;text-decoration:none;color:inherit;display:flex;flex-direction:column;gap:8px}
.bs-kpi:hover{border-color:#9eb1c5}
...
.bs-board__grid{display:grid;grid-template-columns:.9fr 1.4fr;gap:12px}
